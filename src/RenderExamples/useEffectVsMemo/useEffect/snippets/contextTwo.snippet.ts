...
export const ContextTwoProvider: React.FC<{ children: ReactNode }> = ({
    children,
}) => {
    const { textVal } = useAppContextOne();
    const [textValTwo, setTextValTwo] = useState('ContextTwo');
    const [combinedTextValTwo, setTextValTwoCombined] = useState(
        \`\${textVal}-ContextTwo\`,
    );
    ...
    return (
        <AppContext.Provider
            value={{ textValTwo, combinedTextValTwo, setTextValTwo }}
        >
            {children}
        </AppContext.Provider>
    );
